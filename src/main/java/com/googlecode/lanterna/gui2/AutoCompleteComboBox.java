 ...
 * @author Martin
 */
public class AutoCompleteComboBox<V> extends AbstractInteractableComponent<AutoCompleteComboBox<V>> {

    /**
     * Listener interface for when the {@link AutoCompleteComboBox} content has changed. This can be either by user interactions with
    ...
    public interface TextChangeListener {
        /**
         * Callback method invoked by the {@link AutoCompleteComboBox} when the text content has changed
         * @param newText The new content of the {@link AutoCompleteComboBox}
    ...
    protected boolean dropDownFocused;
    protected int textInputPosition;
    protected int dropDownNumberOfRows;
     ...
     */
    @SafeVarargs
    public AutoCompleteComboBox(V... items) {
     ...
     * @param items Items to populate the new combo box with
     */
    public AutoCompleteComboBox(Collection<V> items) {
     ...
     * @param items       Items to populate the new combo box with
     */
    public AutoCompleteComboBox(String initialText, Collection<V> items) {
        this(items, -1);
     ...
     */
    public AutoCompleteComboBox(Collection<V> items, int selectedIndex) {
        for (V item : items) {
            if (item == null) {
     ...
     * @return Itself
     */
    public synchronized AutoCompleteComboBox<V> addItem(V item) {
        if (item == null) {
            throw new IllegalArgumentException("Cannot add null elements to a ComboBox");
        }
        items.add(item);
    ...
    @SafeVarargs
    public final synchronized AutoCompleteComboBox<V> addItems(V... items) {
        for(V i : items) {
            addItem(i);
        }
        return this;
    ...
    public final synchronized AutoCompleteComboBox<V> addItems(Collection<V> items) {
        for(V i : items) {
            addItem(i);
        }
        return this;
     ...
     * @return Itself
     */
    public synchronized AutoCompleteComboBox<V> addItem(int index, V item) {
        if (item == null) {
            throw new IllegalArgumentException("Cannot add null elements to a ComboBox");
        }
     ...
     * @return Itself
     */
    public synchronized AutoCompleteComboBox<V> clearItems() {
        items.clear();
        setSelectedIndex(-1);
        invalidate();
        return this;
     ...
     * @return Itself
     */
    public synchronized AutoCompleteComboBox<V> removeItem(V item) {
     ...
     */
    public synchronized AutoCompleteComboBox<V> removeItem(int index) {
        items.remove(index);
        if (index < selectedIndex) {
            setSelectedIndex(selectedIndex - 1);
     ...
     * @return Itself
     */
    public synchronized AutoCompleteComboBox<V> setItem(int index, V item) {
        if (item == null) {
            throw new IllegalArgumentException("Cannot add null elements to a ComboBox");
        }
     ...
     * @return Itself
     */
    public synchronized AutoCompleteComboBox<V> setReadOnly(boolean readOnly) {
    ...
    public int getSelectedIndex() {
        if (selectedIndex > -1 && !getItem(selectedIndex).toString().equals(getText())) {
            return -1;
        }
     ...
     * @return Itself
     */
    public AutoCompleteComboBox<V> addListener(Listener listener) {
     ...
     * @return Itself
     */
    public AutoCompleteComboBox<V> removeListener(Listener listener) {
     ...
     * @return Itself
     */
    public synchronized AutoCompleteComboBox<V> setTextChangeListener(TextChangeListener textChangeListener) {
    ...
    @Override
    protected InteractableRenderer<AutoCompleteComboBox<V>> createDefaultRenderer() {
    ...
    protected Result handleEditableCBKeyStroke(KeyStroke keyStroke) {
        if (isReadOnly()) {
            return handleEditableCBKeyStrokeOriginal(keyStroke);
        }
        if (popupWindow != null) {
            switch (keyStroke.getKeyType()) {
                case ArrowDown:
        ...
        var res = handleEditableCBKeyStrokeOriginal(keyStroke);
        if (!dropDownFocused && keyStroke.getKeyType() == KeyType.Character
                || keyStroke.getKeyType() == KeyType.Enter
//                || keyStroke.getKeyType() == KeyType.Escape
                || keyStroke.getKeyType() == KeyType.Backspace
        )
            showPopup(new KeyStroke(KeyType.Enter));

        selectItemBasedOnText();
    ...
    private void selectItemBasedOnText() {
        var currentText = text;
        boolean found = false;
        for (int i = 0; i < items.size(); i++) {
            if (items.get(i).toString().equalsIgnoreCase(currentText)) {
        ...
        if (!found) {
            int pos = textInputPosition;
            setSelectedIndex(-1, true);
            text = currentText;
            textInputPosition = pos;
            fireOnTextChanged(true);
        }
    }

    protected Result handleEditableCBKeyStrokeOriginal(KeyStroke keyStroke) {
...
//                return Result.HANDLED;
    ...
    protected class PopupWindow extends BasicWindow {
        ...
        public PopupWindow() {
            setHints(Arrays.asList(
                    ...
                    Hint.FIXED_POSITION,
                    Hint.MENU_POPUP));
            listBox = new ActionListBox(AutoCompleteComboBox.this.getSize().withRows(getItemCount()));
        ...
        public void filterItems(String filter) {
            ...
            if (dropDownNumberOfRows > 0) {
                TerminalSize dropDownListPreferedSize = listBox.getPreferredSize();
                listBox.setPreferredSize(dropDownListPreferedSize.withRows(
                        Math.min(dropDownNumberOfRows, listBox.getItemCount())));
            }
...
//            listBox.setSelectedIndex(getSelectedIndex());
        ...
        public int size() {
            return listBox == null ? 0 : listBox.getItemCount();
        }
        ...
        public synchronized Theme getTheme() {
            return AutoCompleteComboBox.this.getTheme();
        }
        ...
        public void selectNext() {
            int i = Math.min(listBox.getSelectedIndex() + 1, listBox.getItemCount() - 1);
            listBox.setSelectedIndex(i);
        }

        public void selectPrevious() {
            int i = Math.max(listBox.getSelectedIndex() - 1, 0);
            listBox.setSelectedIndex(i);
        }
     ...
     */
    public static abstract class ComboBoxRenderer<V> implements InteractableRenderer<AutoCompleteComboBox<V>> {
    }
    ...
    public static class DefaultComboBoxRenderer<V> extends ComboBoxRenderer<V> {
        ...
        @Override
        public TerminalPosition getCursorLocation(AutoCompleteComboBox<V> comboBox) {
        ...
        @Override
        public TerminalSize getPreferredSize(final AutoCompleteComboBox<V> comboBox) {
        ...
        @Override
        public void drawComponent(TextGUIGraphics graphics, AutoCompleteComboBox<V> comboBox) {
