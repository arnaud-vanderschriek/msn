...
public abstract class Model {
    ...
    private static Object getFromCache(PreparedStatement stmt) {
        ...
        if (cacheDebug) {
            if (entry == null)
                System.out.println(asString(LocalDateTime.now()) + ": Cache Miss: size=" + cache.size());
        }
    ...
    public static void clearCache() {
        if (cacheEnabled) {
            ...
            System.gc();
            if (cacheDebug)
                System.out.println(asString(LocalDateTime.now()) + ": Cache Clear: size=" + cache.size());
        }
    }
    ...
    public static <T extends Model> T queryOne(Class<T> type, String sql, Params params) {
        ...
        try {
            ...
            if (cachedResult != null  && !(cachedResult instanceof ArrayList<?>)) return (T) cachedResult;
    ...
    public static <T extends Model> List<T> queryList(Class<T> type, String sql, Params params) {
        ...
        try {
            var stmt = convertToUnnamedParams(sql, params);
            var cachedResult = getFromCache(stmt);
            if (cachedResult instanceof ArrayList<?>) return (List<T>) cachedResult;
            var rs = stmt.executeQuery();
            while (rs.next()) {
    ...
    public static int execute(String sql, Params params) {
        ...
        try {
            ...
            // On efface le cache des objets du modèle après une mise à jour pour être sûr
            // de retourner en BD lors de la prochaine lecture.
            clearCache();
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
