...
public abstract class Controller<V> {
     ...
     */
    public static <T> void navigateTo(Controller<?> controller) {
        // on efface le cache des objets du modèle à chaque fois qu'on navigue vers un contrôleur
        Model.clearCache();

        Object view = controller.getView();
        if (view instanceof DialogWindow dialogWindow) {
            ViewManager.showModal(dialogWindow);
        } else if (view instanceof Window window) {
            ViewManager.navigateTo(window);
        } else {
    ...
    /**
     * Permet d'afficher une boîte de dialogue modale présentant une information reçue en paramètre.
     * L'information est représentée par un {@code String}.
     *
     * @param info l'information à afficher
     */
    public static void showInfo(String info) {
        showMessage(info, "Information", MessageDialogButton.OK);
    }
     ...
     * @return le bouton que l'utilisateur a utilisé pour fermer la boîte de dialogue de message
     */
    public static String showMessage(String message, String title, String... buttons) {
        return ViewManager.showMessage(message, title, buttons);
    }
